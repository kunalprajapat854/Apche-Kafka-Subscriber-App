package in.kunal.config;

import java.util.HashMap;
import java.util.Map;

import org.apache.kafka.clients.consumer.ConsumerConfig;
import org.apache.kafka.common.serialization.StringDeserializer;
import org.springframework.context.annotation.Bean;
import org.springframework.context.annotation.Configuration;
import org.springframework.kafka.config.ConcurrentKafkaListenerContainerFactory;
import org.springframework.kafka.core.ConsumerFactory;
import org.springframework.kafka.core.DefaultKafkaConsumerFactory;
import org.springframework.kafka.support.serializer.JsonDeserializer;

import in.kunal.binding.Order;
import in.kunal.constants.Appconstansts;

@Configuration
public class SubscriberConfig {
	
	@Bean
	public ConsumerFactory<String, Order> consumerFactory (){
		Map<String, Object> conconfig = new HashMap<String, Object>();
		         conconfig.put(ConsumerConfig.BOOTSTRAP_SERVERS_CONFIG, Appconstansts.Host);
		         conconfig.put(ConsumerConfig.KEY_DESERIALIZER_CLASS_CONFIG, StringDeserializer.class);
		         conconfig.put(ConsumerConfig.VALUE_DESERIALIZER_CLASS_CONFIG, JsonDeserializer.class);
		         return new DefaultKafkaConsumerFactory<>(conconfig);
	}
	
	@Bean
	public ConcurrentKafkaListenerContainerFactory<String ,Order> kafkalistner(){
		  ConcurrentKafkaListenerContainerFactory<String, Order> kafkafactory = new ConcurrentKafkaListenerContainerFactory<>();
		   kafkafactory.setConsumerFactory(consumerFactory());
		   return kafkafactory;
	}

}
